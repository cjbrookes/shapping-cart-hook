...
export function CartProvider({ children }: CartProviderProps): JSX.Element {
  ...
  const addProduct = async (productId: number) => {
    try {
      ...
      if (productStock.amount <= 1) {   //check if out of stock
        throw new Error('Quantidade solicitada fora de estoque'); 
        }

      let existingProduct = cart.find(product => product.id === productId) //check if product id is same as id received by function
      let newCart: Product[] = []
  ...
  const removeProduct = async (productId: number) => {
    try {
      const product = cart.find(product => product.id === productId) //check if product is in cart
